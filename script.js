...
window.addEventListener('scroll', () => {
    if (window.innerWidth <= 768) {
        ...
        if (modesContainer) {
            const rect = modesContainer.getBoundingClientRect();
            const elementCenter = rect.top + rect.height / 2;
            const windowCenter = window.innerHeight / 2;
            
            modeCards.forEach((card, index) => {
                const scrollAmount = Math.max(0, windowCenter - elementCenter);
                if (index === 0) {
                    card.style.transform = `translateY(${Math.min(scrollAmount * 0.5, 80)}px)`;
                    card.style.opacity = Math.max(0.6, 1 - (scrollAmount / window.innerHeight) * 0.4);
                } else if (index === 1) {
                    card.style.transform = `translateY(${Math.min(scrollAmount * 0.3, 50)}px)`;
                    card.style.opacity = Math.max(0.7, 1 - (scrollAmount / window.innerHeight) * 0.3);
                }
            });
        }
    }
