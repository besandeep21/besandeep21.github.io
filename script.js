...
// ===== CAROUSEL NAVIGATION - DESKTOP & MOBILE =====
...
if (carouselContainer && carouselPrev && carouselNext) {
    ...
    // Arrow click handlers - DESKTOP FIX
...
// ===== MODE CARDS SCROLL ANIMATION - MOBILE ONLY =====
const modesSection = document.querySelector('.modes');
const modeCards = document.querySelectorAll('.mode-card');

if (modesSection && modeCards.length > 0) {
    function updateModeCardsAnimation() {
        if (window.innerWidth > 768) return; // Desktop - no animation

        const sectionRect = modesSection.getBoundingClientRect();
        const sectionTop = sectionRect.top;
        const sectionHeight = sectionRect.height;
        const windowHeight = window.innerHeight;

        // Calculate scroll progress within the modes section
        // 0 = section at bottom of viewport, 1 = section at top
        let scrollProgress = 1 - (sectionTop / windowHeight);
        scrollProgress = Math.max(0, Math.min(1, scrollProgress));

        // Online card animation: appears after 30% scroll
        const onlineCard = modeCards[1];
        const onlineProgress = Math.max(0, (scrollProgress - 0.3) / 0.35);
        onlineCard.style.opacity = onlineProgress;
        onlineCard.style.transform = `translateY(${(1 - onlineProgress) * 200}px)`;
        
        if (onlineProgress > 0.5) {
            onlineCard.classList.add('visible');
        } else {
            onlineCard.classList.remove('visible');
        }

        // Hybrid card animation: appears after 65% scroll
        const hybridCard = modeCards[2];
        const hybridProgress = Math.max(0, (scrollProgress - 0.65) / 0.35);
        hybridCard.style.opacity = hybridProgress;
        hybridCard.style.transform = `translateY(${(1 - hybridProgress) * 200}px)`;
        
        if (hybridProgress > 0.5) {
            hybridCard.classList.add('visible');
        } else {
            hybridCard.classList.remove('visible');
        }
    }

    window.addEventListener('scroll', updateModeCardsAnimation);
    // Initial call
    updateModeCardsAnimation();
}
